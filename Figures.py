...
def select_high_variance_blocks(image, block_size=8, top_ratio=0.25):
    ...
    Returns list of ((i, j), variance) for the top 'top_ratio' blocks.
    """
    h, w = image.shape
    blocks_info = []
...
def figure_3(image_path="original.png", block_coord=(0, 0), block_size=8, Q=5, watermark_bit=1):
    ...
    matrices before and after watermark embedding.
    """
    # Load image in grayscale and convert to float32
    image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if image is None:
        raise ValueError("Image not found")
    image = image.astype(np.float32)

    # Select the block at block_coord
    top, left = block_coord
    block = image[top:top+block_size, left:left+block_size]

    # Compute DCT of the original block
    ...
    # For demonstration, choose one coefficient index using logistic map (or fixed index)
    # Here we choose a fixed index (e.g., index 0) for clarity.
    idx = 0
    C = flat_dct_mod[idx]
    # QIM embedding: use rounding to embed watermark_bit
    u = int(round(C / Q))
    ...
    # Reshape back to block and compute inverse DCT to check visual impact (optional)
    dct_modified = flat_dct_mod.reshape((block_size, block_size))
    block_modified = apply_idct(dct_modified)

    # Plot the original and modified DCT coefficient matrices
...
def figure_5(image_path="original.png", block_size=8, top_ratio=0.25):
    ...
    # Create a matrix to store variance values
    ...
    # Determine top high-variance blocks
    ...
    # Also, overlay rectangles on the original image to show selected blocks
    ...
    for (i, j), _ in top_blocks:
        # Draw rectangle on original image (blue rectangle)
...
if __name__ == "__main__":
    # Figure 3: DCT coefficient matrix before and after embedding
    ...
    # Figure 5: Heatmap of variance and highlight high-variance blocks
